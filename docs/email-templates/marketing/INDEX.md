...
- **Opens**: the pixel `https://tekmadev.com/api/e/o?c=<key>` logs an open when the email is viewed.
- **Clicks**: each CTA points to `https://tekmadev.com/api/e/c?c=<key>&l=<label>&u=<destination>`, which logs the click and forwards to the destination (destinations are host-allowlisted, so the link can't be abused as an open redirect).
